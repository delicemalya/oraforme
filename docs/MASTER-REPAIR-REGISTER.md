...
| R006-SEED-SCRIPT-NO-GUARD | `scripts/seed-demo-data.ts` reste exécutable tel quel contre n'importe quel `.env.local` pointant vers production, ciblant automatiquement « le tenant le plus ancien » — mécanisme exact de l'incident nettoyé par la migration 179, toujours actif | **OPEN** — MEDIUM (risque de récidive, indépendant de la recette) | Lecture de code (R-006, 2026-09-08) | NEW-06 (incident historique déjà nettoyé) |
...
| Nouveaux problèmes découverts (hors des 72, Tables 2+4) | **19** (NEW-01→06, R004-xx ×4, R005-xx ×4, R006-xx ×5) — dont 3 requalifiés « risque latent, pas incident actif », 1 VERIFIED, **1 CLOSED (NEW-06)**, et 9 nouveaux `OPEN` ajoutés le 2026-09-08 (4 R005 + 5 R006), dont 2 marqués **BLOCKED** (R006-MIGRATION-PHANTOM-TABLES, R006-REPAIR-MIGRATIONS-NOT-REPLAYABLE) |
...
| **Total réel restant à traiter (`OPEN`+`PRODUCTION_PENDING`, tables 1+2+4)** | 56 (table 1) + 5 (table 2 : NEW-01→05, NEW-06 exclue car `CLOSED`) + 11 (table 4 : 2 R004 + 4 R005 + 5 R006, `VERIFIED`/`NOT_TESTABLE` exclus) = **72** au 2026-09-08, dont 2 marquées `BLOCKED` |
