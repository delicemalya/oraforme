...
| P0A-SEED-EMPLOYES-TEST-HARDCODED-PROD | `scripts/seed-employes-test.mjs` : URL Supabase de production codée en dur dans le source (`mrzixapnaqsbqmagivvf.supabase.co`, confirmée identique à `.env.local` prod), `DELETE` inconditionnel sur `employes` exécuté au chargement du module (hors de toute fonction), aucun garde d'environnement — plus dangereux que R006-SEED-SCRIPT-NO-GUARD car indépendant du `.env.local` de qui l'exécute | **OPEN — signalé, non corrigé (hors périmètre P0-A qui portait uniquement sur seed-demo-data.ts)** | Audit exhaustif `scripts/` (P0-A, 2026-09-07) | — |
...
| Nouveaux problèmes découverts (hors des 72, Tables 2+4) | **20** (NEW-01→06, R004-xx ×4, R005-xx ×4, R006-xx ×5, P0A-xx ×1) — dont 3 requalifiés « risque latent, pas incident actif », 1 VERIFIED, **1 CLOSED (NEW-06)**, **1 CODE_FIXED le 2026-09-07 (R006-SEED-SCRIPT-NO-GUARD, mission P0-A)**, et 1 nouveau `OPEN` ajouté le 2026-09-07 (P0A-SEED-EMPLOYES-TEST-HARDCODED-PROD, signalé non corrigé), dont 2 marqués **BLOCKED** (R006-MIGRATION-PHANTOM-TABLES, R006-REPAIR-MIGRATIONS-NOT-REPLAYABLE) |
...
| **Total réel restant à traiter (`OPEN`+`PRODUCTION_PENDING`, tables 1+2+4)** | 56 (table 1) + 5 (table 2 : NEW-01→05, NEW-06 exclue car `CLOSED`) + 11 (table 4 : 2 R004 + 4 R005 + 4 R006 [R006-SEED-SCRIPT-NO-GUARD exclue, `CODE_FIXED`] + 1 P0A) = **72** au 2026-09-07, dont 2 marquées `BLOCKED` |
