  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 bg-white">
      <div className="w-full max-w-md">
        {/* Back button */}
        <div className="mb-4">
          <Link href="/" className="inline-flex items-center gap-1.5 text-sm text-[#4B5563] hover:text-[#111827] transition-colors">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[#4B5563] text-sm">Connectez-vous à votre espace</p>
        </div>
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-xl p-8">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 px-4 py-2.5 rounded-lg border border-[#E2E8F0] bg-[#F0F4FF] text-[#111827] text-sm font-medium hover:border-[#8B0073] hover:bg-[#E2EAFA] transition-all disabled:opacity-60 disabled:cursor-not-allowed mb-5"
          >
            <GoogleIcon />
            ...
            <form onSubmit={handleEmailLogin} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium text-[#4B5563]">Mot de passe</label>
          ...
          {mode === 'phone' && phoneStep === 'enter' && (
            <form onSubmit={handleSendOtp} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-[#4B5563]">Numéro de téléphone</label>
                <div className="flex gap-2">
                  <div className="px-3 py-2.5 rounded-lg bg-[#F0F4FF] border border-[#E2E8F0] text-[#4B5563] text-sm shrink-0 flex items-center">
                    🇨🇬 +242
                  </div>
                  <input
                    type="tel"
                    ...
                    className="flex-1 px-4 py-2.5 rounded-lg bg-[#F0F4FF] border border-[#E2E8F0] text-[#111827] text-sm focus:outline-none focus:border-[#F0A30A] placeholder-[#9CA3AF]"
                    required
                  />
                </div>
          ...
          {mode === 'phone' && phoneStep === 'otp' && (
            <form onSubmit={handleVerifyOtp} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-[#4B5563]">Code de vérification</label>
                <input
                  type="text"
                  inputMode="numeric"
                  pattern="[0-9]*"
                  ...
                  maxLength={6}
                  className="w-full px-4 py-3 text-center text-2xl tracking-[0.5em] rounded-lg bg-[#F0F4FF] border border-[#E2E8F0] text-[#111827] focus:outline-none focus:border-[#F0A30A] placeholder-[#9CA3AF]"
                  required
                />
              </div>
              {error && <ErrorBox message={error} />}
              ...
              <button
                ...
                className="text-xs text-[#4B5563] hover:text-[#111827] text-center"
              >
                ← Changer de numéro
              </button>
            </form>
