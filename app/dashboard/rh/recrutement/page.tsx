  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-[#0F172A]">Recrutement IA</h1>
          <p className="text-xs text-[#64748B] mt-0.5">Propulsé par MIAA Job</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            <Star size={14} className="text-[#F59E0B]" /><h2 className="text-sm font-bold text-[#0F172A]">Top Talents</h2>
          </div>
          {talents.length === 0 ? <p className="text-xs text-[#94A3B8] text-center py-8">Aucun talent scoré</p> : (
            <div className="space-y-3">
              {talents.map((t, i) => (
                <div key={t.id} className="flex items-center gap-3">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0"
                    style={{ background: i === 0 ? '#F59E0B' : i === 1 ? '#94A3B8' : '#CD7F32' }}>{i + 1}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-[#0F172A] truncate">{[t.prenom, t.nom].filter(Boolean).join(' ') || 'Candidat'}</p>
                    <p className="text-[10px] text-[#64748B] truncate">{(t.competences ?? []).slice(0, 2).join(', ') || '—'}</p>
                  </div>
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${scoreCls(t.score_global)}`}>{t.score_global}/100</span>
                </div>
              ))}
            </div>
          )}
          <button onClick={() => router.push('/dashboard/rh/recrutement/cvtheque')} className="mt-5 w-full text-xs text-[#F59E0B] font-semibold hover:text-[#E09000]">Voir tous →</button>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-[#E2E8F0] p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => router.push('/dashboard/rh/recrutement/offres')} className="text-xs text-[#F59E0B] font-semibold hover:text-[#E09000]">Gérer →</button>
          </div>
          {offres.length === 0 ? <p className="text-xs text-[#94A3B8] text-center py-8">Aucune offre active</p> : (
            <div className="divide-y divide-[#F1F5F9]">
              {offres.map(o => (
                <div key={o.id} className="flex items-center gap-3 py-3">
                  ...
                  {o.scoreMoyen > 0 && <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${scoreCls(o.scoreMoyen)}`}>Moy. {o.scoreMoyen}</span>}
                  <button onClick={() => router.push(`/dashboard/rh/recrutement/candidatures?offre=${o.id}`)} className="text-[10px] text-[#2563EB] font-semibold hover:underline shrink-0">TOP 5</button>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-[#E2E8F0] p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => router.push('/dashboard/rh/recrutement/candidatures')} className="text-xs text-[#F59E0B] font-semibold hover:text-[#E09000]">Voir tout →</button>
          </div>
          {recentes.length === 0 ? <p className="text-xs text-[#94A3B8] text-center py-8">Aucune candidature</p> : (
            <div className="divide-y divide-[#F1F5F9]">
              {recentes.map(c => (
                <div key={c.id} className="flex items-center gap-3 py-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-[#0F172A] truncate">
                      ...
                      {c.est_doublon && <span className="ml-1.5 text-[9px] bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded-full">Doublon</span>}
                    </p>
                    <p className="text-[10px] text-[#64748B] truncate">{c.offre_titre ?? '—'}</p>
                  </div>
                  {c.score_ia > 0
                    ? <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${scoreCls(c.score_ia)}`}>{c.score_ia} · {scoreCat(c.score_ia)}</span>
                    : <span className="text-[10px] text-[#94A3B8] shrink-0">Non analysé</span>
                  }
                  <span className="text-[10px] text-[#CBD5E1] shrink-0">{new Date(c.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</span>
                </div>
              ))}
            </div>
          )}
        ...
        {/* ALERTES */}
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-4"><AlertTriangle size={14} className="text-[#F59E0B]" /><h2 className="text-sm font-bold text-[#0F172A]">Alertes</h2></div>
            ...
            <div className="space-y-3">
              {doublons > 0 && (
                <div className="rounded-lg p-3 bg-orange-50 border border-orange-100">
                  <p className="text-xs font-bold text-orange-700">{doublons} doublon{doublons > 1 ? 's' : ''} détecté{doublons > 1 ? 's' : ''}</p>
                  <button onClick={() => router.push('/dashboard/rh/recrutement/candidatures')} className="text-[10px] text-orange-600 font-semibold underline mt-1">Voir →</button>
                </div>
              )}
              {nonAnalyses > 0 && (
                <div className="rounded-lg p-3 bg-blue-50 border border-blue-100">
                  <p className="text-xs font-bold text-blue-700">{nonAnalyses} CV non analysé{nonAnalyses > 1 ? 's' : ''}</p>
                  <button onClick={() => router.push('/dashboard/rh/recrutement/cvtheque')} className="text-[10px] text-blue-600 font-semibold underline mt-1">Analyser →</button>
                </div>
              )}
            </div>
          )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'MIAA+ IA', href: '/dashboard/rh/recrutement/miaa-job', icon: TrendingUp, sub: 'Assistant' },
