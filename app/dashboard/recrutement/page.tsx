...
import {
  Briefcase, Users, Calendar, CheckCircle, Star, AlertTriangle,
  TrendingUp, Loader2, Plus, Clock, Target, UserCheck,
...
interface TopTalent {
  ...
  score_global: number; competences: string[] | null
}
interface OffreStats {
  id: string; titre: string; nb: number; scoreMoyen: number
}
...
const STATUT_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  ...
  place:      { bg: 'bg-teal-50',   text: 'text-teal-700',   label: 'Placé'     },
}

export default function RecrutementDirection() {
  const router = useRouter()
  const { tenantId, loading: tl } = useTenant()
  const [kpis, setKpis] = useState({
    offresActives: 0, candidaturesMois: 0, entretiens: 0,
    retenus: 0, placementsMois: 0, tauxTransformation: 0,
  })
  ...
  async function load() {
    ...
    const [
      { count: oa }, { count: cm }, { count: ent },
      { count: ret }, { count: dbl }, { count: na },
    ] = await Promise.all([
    ...
    const taux = cm && cm > 0 ? Math.round(((ret ?? 0) / cm) * 100) : 0
    setKpis({
      offresActives: oa ?? 0, candidaturesMois: cm ?? 0, entretiens: ent ?? 0,
      retenus: ret ?? 0, placementsMois: 0, tauxTransformation: taux,
    })
    setDoublons(dbl ?? 0)
    setNonAnalyses(na ?? 0)

    const { data: t } = await supabase
      .from('candidats').select('id, nom, prenom, score_global, competences')
      .eq('tenant_id', tenantId).gt('score_global', 0)
      .order('score_global', { ascending: false }).limit(3)
    setTalents(t ?? [])

    const { data: ov } = await supabase
      .from('offres_emploi').select('id, titre, candidatures(id, score_ia)')
    ...
    const { data: rc } = await supabase
      .from('candidatures')
      .select('id, created_at, statut, score_ia, est_doublon, candidats(nom, prenom), offres_emploi(titre)')
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-[18px] sm:text-xl font-bold text-[#0F172A] leading-tight">
            Direction RH
          </h1>
          <p className="text-[11px] text-[#64748B]">Recrutement & Placement — Tableau de bord</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        {[
          { icon: Briefcase,   label: 'Offres actives',        value: kpis.offresActives,      color: '#2563EB', suffix: '' },
          { icon: Users,       label: 'Candidatures ce mois',  value: kpis.candidaturesMois,   color: '#F59E0B', suffix: '' },
          { icon: Calendar,    label: 'Entretiens planifiés',   value: kpis.entretiens,          color: '#8B5CF6', suffix: '' },
          { icon: UserCheck,   label: 'Candidats retenus',     value: kpis.retenus,             color: '#16A34A', suffix: '' },
          { icon: Target,      label: 'Placements ce mois',    value: kpis.placementsMois,      color: '#0EA5E9', suffix: '' },
          { icon: TrendingUp,  label: 'Taux transformation',   value: kpis.tauxTransformation,  color: '#EC4899', suffix: '%' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 sm:p-5 shadow-sm">
          ...
          <button
            onClick={() => router.push('/dashboard/recrutement/cvtheque')}
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-[#E2E8F0] p-4 sm:p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => router.push('/dashboard/recrutement/offres')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-[#E2E8F0] p-4 sm:p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => router.push('/dashboard/recrutement/candidatures')}
              className="text-xs text-[#F59E0B] font-semibold hover:text-[#D97706]"
            >Voir tout →</button>
          </div>
          {recentes.length === 0 ? (
            ...
            <div className="space-y-3">
              {doublons > 0 && (
                <div className="rounded-xl p-3 bg-orange-50 border border-orange-100">
                  ...
                  <button onClick={() => router.push('/dashboard/recrutement/candidatures')} className="text-[10px] text-orange-600 font-semibold underline mt-1">
                    Voir →
                  </button>
                </div>
              )}
              {nonAnalyses > 0 && (
                <div className="rounded-xl p-3 bg-blue-50 border border-blue-100">
                  ...
                  <button onClick={() => router.push('/dashboard/recrutement/cvtheque')} className="text-[10px] text-blue-600 font-semibold underline mt-1">
                    Analyser →
                  </button>
                </div>
              )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Offres',        href: '/dashboard/recrutement/offres',          icon: Briefcase,  sub: 'Gérer les postes'   },
          { label: 'Candidatures',  href: '/dashboard/recrutement/candidatures',    icon: Users,      sub: 'Pipeline complet'   },
          { label: 'Placements',    href: '/dashboard/recrutement/placement',       icon: UserCheck,  sub: 'Missions placées'   },
          { label: 'Analytics',     href: '/dashboard/recrutement/analytics',       icon: BarChart2,  sub: 'KPIs & rapports'    },
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {[
          { label: 'Entretiens',   href: '/dashboard/recrutement/entretiens',   icon: Calendar,   color: '#8B5CF6' },
          { label: 'CV-thèque',    href: '/dashboard/recrutement/cvtheque',     icon: Star,       color: '#F59E0B' },
          { label: 'Contrats',     href: '/dashboard/recrutement/contrats',     icon: CheckCircle,color: '#16A34A' },
          { label: 'Partenaires',  href: '/dashboard/recrutement/partenaires',  icon: Clock,      color: '#0EA5E9' },
          { label: 'MIAA+ Job',    href: '/dashboard/miaa?context=recrutement', icon: TrendingUp, color: '#DC2626' },
        ].map(({ label, href, icon: Icon, color }) => (
          <button
            key={href}
            onClick={() => router.push(href)}
            className="bg-white border border-[#E2E8F0] rounded-xl p-3 text-left hover:shadow-md transition-all group flex items-center gap-3 shadow-sm"
          >
            <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors" style={{ background: color + '18' }}>
              <Icon size={14} style={{ color }} />
            </div>
            <div className="min-w-0">
              <p className="text-xs font-semibold text-[#0F172A] truncate">{label}</p>
              <ArrowRight size={10} className="text-[#94A3B8] mt-0.5" />
            </div>
          </button>
        ))}
      </div>
