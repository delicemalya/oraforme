...
const TABS: ERPTab[] = [
  ...
  { id: 'clients',   label: 'Clients actifs' },
...
export default function CRMPage() {
  ...
  const kpis: ERPKpi[] = [
    { label: 'Clients actifs',    value: nbClients,        color: '#16A34A', icon: <Users size={16} /> },
    { label: 'Prospects',         value: nbProspects,      color: '#F59E0B', icon: <TrendingUp size={16} /> },
    { label: 'Clients inactifs',  value: nbInactifs,       color: '#64748B', icon: <Building2 size={16} /> },
    { label: "CA généré",         value: fmtFCFA(caTotal), color: '#2563EB', icon: <TrendingUp size={16} /> },
  ]

  const tabsWithBadge: ERPTab[] = [
    { id: 'tous',      label: 'Tous les clients', badge: clients.length },
    { id: 'clients',   label: 'Clients actifs',   badge: nbClients },
    { id: 'prospects', label: 'Prospects',         badge: nbProspects },
    { id: 'inactifs',  label: 'Inactifs',          badge: nbInactifs },
  ]
  ...
  const columns: ERPColumn<Client>[] = [
    ...
    { key: 'type',       label: 'Type',        render: r => (
      <ERPStatusBadge
        ...
        label={r.type === 'client' ? 'Client' : r.type === 'prospect' ? 'Prospect' : 'Inactif'}
      />
    )},
    { key: 'ca_total',   label: 'CA généré',   render: r => (
      <span className="font-semibold text-[#0F172A] tabular-nums text-[13px]">
        {r.ca_total ? fmtFCFA(r.ca_total) : '—'}
      </span>
    )},
    { key: 'created_at', label: "Date d'entrée", render: r => (
  ...
  return (
    <ERPPageLayout
      title="CRM Clients"
      subtitle="Relations clients & prospects"
      icon={<Users size={18} />}
      color="#F59E0B"
      kpis={kpis}
      tabs={tabsWithBadge}
      ...
      {showNew && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <h2 className="text-[16px] font-bold text-[#0F172A] mb-4">Nouveau client / prospect</h2>
