...
function SectionEmployes({ tenantId }: { tenantId: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data } = await supabase.from('employes').select('*').eq('tenant_id', tenantId).order('nom').limit(200)
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {displayed.map((e, idx) => {
            ...
            const sc = e.statut === 'actif'
              ...
              : e.statut === 'suspendu'
              ? { color: '#FFFFFF', bg: '#DC2626', border: 'none', label: 'SUSPENDU' }
            ...
            return (
              <motion.div key={e.id} layout
                ...
                <div className="space-y-1.5 pt-1 border-t border-[var(--border)]">
                  ...
                  <div className="flex items-center gap-2">
                    <DollarSign size={11} className="text-[#DC2626] shrink-0" />
                    <p className="text-[11px] font-semibold" style={{ color: '#DC2626' }}>{fmt(brut)} FCFA/mois</p>
                  </div>
                </div>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={() => { setShowForm(false); setForm(EMPTY_ENS_FORM); setPhotoFile(null); setPhotoPreview(null) }} className="px-4 py-2 rounded-lg text-xs text-[var(--text-secondary)] border border-[var(--border)]">{t('common.cancel')}</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function SectionStaff({ tenantId }: { tenantId: string }) {
  ...
  const load = useCallback(async () => {
    const { data } = await supabase.from('staff_ecole').select('*').eq('tenant_id', tenantId).order('nom').limit(200)
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={() => { setShowForm(false); setForm(EMPTY_STAFF); setPhotoFile(null); setPhotoPreview(null) }} className="px-4 py-2 rounded-lg text-xs text-[var(--text-secondary)] border border-[var(--border)]">{t('common.cancel')}</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
            ...
            return (
              <motion.div key={s.id} layout
                ...
                <div className="pt-2 border-t border-[var(--border)] text-center">
                  ...
                  <p className="text-sm font-bold mt-0.5" style={{ color: '#DC2626' }}>{fmt(s.salaire)} FCFA</p>
                </div>
...
function SectionConges({ tenantId, enseignants }: { tenantId: string; enseignants: Enseignant[] }) {
  ...
  const load = useCallback(async () => {
    const { data } = await supabase.from('conges_ecole').select('*').eq('tenant_id', tenantId).order('date_debut', { ascending: false }).limit(200)
  ...
  const STATUT_COLORS: Record<string, { color: string; bg: string; border: string }> = {
    en_attente: { color: '#DC2626', bg: 'rgba(245,30,51,0.15)',   border: '1px solid #DC2626' },
    approuve:   { color: '#FFFFFF', bg: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.2)' },
    refuse:     { color: '#DC2626', bg: 'rgba(245,30,51,0.15)',   border: '1px solid #DC2626' },
  }
                    ...
                    <td className="px-4 py-2.5">
                      {c.statut === 'en_attente' && (
                        <div className="flex gap-1">
                          ...
                          <button onClick={() => updateStatut(c.id, 'refuse')}   className="px-2 py-1 rounded text-[10px] font-semibold" style={{ background: '#DC262618', color: '#DC2626' }}>Refuser</button>
                        </div>
                      )}
                    </td>
                  </tr>
...
function SectionPaie({ tenantId, nomEcole }: { tenantId: string; nomEcole: string }) {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: emp }, { data: ens }, { data: stf }, { data: pai }] = await Promise.all([
      supabase.from('employes').select('*').eq('tenant_id', tenantId).eq('statut', 'actif').order('nom').limit(200),
      supabase.from('enseignants').select('*').eq('tenant_id', tenantId).eq('statut', 'actif').eq('type_enseignant', 'employe').order('nom').limit(200),
      supabase.from('staff_ecole').select('*').eq('tenant_id', tenantId).eq('statut', 'actif').order('nom').limit(200),
      supabase.from('paie_ecole').select('*').eq('tenant_id', tenantId).order('created_at', { ascending: false }).limit(200),
    ])
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-[var(--border)] p-5 space-y-5" style={{ background: 'rgba(255,255,255,0.02)' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            <label className="block text-xs text-[var(--text-secondary)] mb-1">{t('common.month')}</label>
                  ...
                  {MODES_PAIE.map(m => {
                    ...
                    return (
                      <button key={m.id} onClick={() => payer(m.id)} disabled={paying}
                        ...
                        style={{ background: 'rgba(245,30,51,0.08)', borderColor: 'rgba(245,30,51,0.3)', color: '#DC2626' }}>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            style={{ background: toast.ok ? '#7C3AED' : '#DC2626' }}>
        ...
        <div className="space-y-2">
          {postes.map(p => (
            <div key={p.id} className="rounded-xl border border-[var(--border)] p-4 flex items-start gap-4" style={{ background: 'rgba(255,255,255,0.02)' }}>
              <AlertCircle size={14} className="mt-0.5 shrink-0" style={{ color: p.statut === 'ouvert' ? '#DC2626' : 'rgba(255,255,255,0.35)' }} />
      ...
      <div className="flex gap-1">
        {(['tous', 'declare', 'validated', 'paye'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={filter === f ? { background: f === 'tous' ? '#0F172A' : STATUT_CFG[f]?.color ?? '#0F172A' } : {}}>
                ...
                <div className="flex gap-1.5">
                  {h.statut === 'declare' && (
                    <button onClick={() => valider(h)} disabled={saving === h.id}
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-semibold disabled:opacity-40" style={{ background: '#7C3AED', color: '#fff' }}>
