  ...
  return (
    <div className={className}>
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)}
        ...
        className="w-full bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-white placeholder-[#9CA3AF] focus:outline-none focus:border-white/[0.2] transition-colors" />
    </div>
  )
}
  ...
  return (
    <div className={className}>
      ...
      <select value={value} onChange={e => onChange(e.target.value)}
        className="w-full bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-white/[0.2] transition-colors">
...
export function CreateEmployeeWizard({
  ...
  const SEC = 'text-[10px] font-semibold text-[#6B7280] uppercase tracking-wider mb-3'
        ...
        <div className="shrink-0 px-6 pt-5 pb-4 border-b border-white/[0.06]">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-[#4B5563] mt-0.5">Étape {step} / {STEPS.length} — {STEPS[step - 1].label}</p>
            </div>
            <button onClick={onClose} className="p-2 rounded-lg text-[#6B7280] hover:text-white transition-colors">
              <X size={16} />
            </button>
          </div>
              ...
              return (
                <div key={n} className="flex flex-col items-center gap-1">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center transition-all"
                    style={{
                      background: past ? '#2EA043' : active ? 'rgba(46,160,67,0.12)' : 'rgba(255,255,255,0.04)',
            ...
            <motion.div key={step}
              ...
              {step === 3 && (
                <div className="space-y-5">
                  <div>
                    ...
                    <p className="text-[10px] text-[#6B7280] mt-1.5">Laissez vide si l'employé n'a pas accès au système.</p>
                  </div>
                      ...
                      {ROLES_LIST.map(r => (
                        <div key={r.id} onClick={() => toggleRole(r.id)}
                          ...
                          <div className="w-4 h-4 rounded flex items-center justify-center shrink-0 transition-all"
                            style={{ background: acc_.roles[r.id] ? '#2EA043' : 'rgba(255,255,255,0.08)' }}>
                            {acc_.roles[r.id] && <Check size={10} className="text-white" />}
                          </div>
                          <span className="text-xs" style={{ color: acc_.roles[r.id] ? '#fff' : '#8B949E' }}>{r.label}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <div className="mt-3 p-3 rounded-xl" style={{ background: 'rgba(46,160,67,0.06)', border: '1px solid rgba(46,160,67,0.15)' }}>
                      <p className="text-[10px] text-[#4B5563] uppercase tracking-wider">Salaire brut estimé</p>
                      ...
                      {MODES_PAIE.map(m => (
                        <button key={m} type="button" onClick={() => setSal(p => ({ ...p, mode_paiement: m }))}
                          ...
                          style={sal_.mode_paiement === m
                            ...
                            : { background: 'rgba(255,255,255,0.04)', color: '#8B949E', border: '1px solid rgba(255,255,255,0.06)' }}>
                          {m.replace('_', ' ')}
                        </button>
                      ))}
                    </div>
              ...
              {step === 5 && (
                <div className="space-y-3">
                  <p className="text-xs text-[#4B5563]">Documents attachés au dossier et stockés de manière sécurisée.</p>
                  {[
                    ...
                    <label key={doc.label}
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[10px] text-[#6B7280] mt-0.5">{doc.hint}</p>
                      </div>
                      <div className="flex items-center gap-1.5 text-[#4B5563] group-hover:text-white transition-colors">
                        <Upload size={13} /><span className="text-xs">Choisir</span>
                      </div>
                      <input type="file" accept=".pdf,image/*" className="hidden" />
                    </label>
                  ))}
                  <p className="text-[10px] text-[#6B7280]">Stockage de documents disponible dans une prochaine mise à jour.</p>
                </div>
              )}
                  ...
                  {done ? (
                    <div className="flex flex-col items-center justify-center py-14 gap-4">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-xs text-[#4B5563] mt-1">Dossier RH · Journal comptable · Direction notifiée</p>
                      </div>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="grid grid-cols-3 gap-2.5">
                        {[
                          ...
                          <div key={l} className="p-3 rounded-lg"
                            ...
                            <p className="text-[9px] text-[#6B7280] uppercase tracking-wider">{l}</p>
                      ...
                      <div className="rounded-xl p-4"
                        ...
                        <p className="text-xs font-semibold text-[#8B0073] mb-2">Workflow automatique à la création</p>
        ...
        {!done && (
          ...
          <div className="flex items-center justify-between">
            <button onClick={() => step > 1 ? setStep(s => s - 1) : onClose()}
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-xs text-[#4B5563] border border-white/[0.08] hover:text-white transition-colors">
              <ChevronLeft size={13} /> {step > 1 ? 'Précédent' : 'Annuler'}
            </button>

            <span className="text-[10px] text-[#6B7280]">{step} / {STEPS.length}</span>
