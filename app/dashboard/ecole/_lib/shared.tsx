...
export const STATUT_ETU: Record<StatutEtu, { label: string; color: string; bg: string }> = {
  ...
  banni:    { label: 'Banni',    color: '#F85149', bg: '#F8514918' },
  diplome:  { label: 'Diplômé', color: '#8B5CF6', bg: '#8B5CF618' },
}
...
export const TYPE_EVENT: Record<TypeEvent, { label: string; color: string; bg: string }> = {
  examen:         { label: 'Examen',    color: '#F85149', bg: '#F8514918' },
  conge_scolaire: { label: 'Congé',     color: '#388BFD', bg: '#388BFD18' },
  evenement:      { label: 'Événement', color: '#2EA043', bg: '#2EA04318' },
  conseil:        { label: 'Conseil',   color: '#8B5CF6', bg: '#8B5CF618' },
...
<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Times New Roman',serif;background:#fff}.page{width:880px;height:620px;margin:auto;padding:48px 60px;border:8px double #8B5CF6;position:relative;display:flex;flex-direction:column;align-items:center;text-align:center;background:linear-gradient(135deg,#fdfcfe 0%,#f5f3ff 100%)}.watermark{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:120px;color:#8B5CF620;font-weight:900;letter-spacing:-4px;pointer-events:none}.header{margin-bottom:24px}.school{font-size:22px;font-weight:700;text-transform:uppercase;letter-spacing:4px;color:#1a1a2e}.title{font-size:36px;font-weight:700;text-transform:uppercase;letter-spacing:6px;color:#8B5CF6;margin:16px 0;border-top:2px solid #8B5CF6;border-bottom:2px solid #8B5CF6;padding:8px 0}.name{font-size:28px;font-style:italic;font-weight:700;color:#1a1a2e;margin-bottom:8px}.level{font-size:16px;color:#444;margin-bottom:4px}.mention{font-size:20px;font-weight:700;color:${mention?.color ?? '#2EA043'};margin:16px 0}.details{font-size:12px;color:#666;line-height:2}.footer{display:flex;justify-content:space-between;width:100%;margin-top:auto;padding-top:24px}.sign-box{text-align:center;width:200px}.sign-line{border-top:1px solid #555;padding-top:6px;font-size:10px;text-transform:uppercase;letter-spacing:1px;color:#555}.num{position:absolute;bottom:16px;right:24px;font-size:9px;color:#aaa}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style>
</head><body><div class="page">
...
const COLOR_GRADIENTS: Record<string, string> = {
  ...
  '#388BFD': 'linear-gradient(135deg,#1E3A5F 0%,#1D4ED8 50%,#3B82F6 100%)',
  '#F0A30A': 'linear-gradient(135deg,#78350F 0%,#D97706 50%,#F59E0B 100%)',
  '#F85149': 'linear-gradient(135deg,#7C1D1D 0%,#B91C1C 50%,#EF4444 100%)',
  '#8B5CF6': 'linear-gradient(135deg,#4C1D95 0%,#7C3AED 50%,#8B5CF6 100%)',
