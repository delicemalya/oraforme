    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
      ...
      <div className="rounded-xl border p-5" style={{ borderColor: accentColor + '30', background: accentColor + '08' }}>
        <div className="flex items-start justify-between">
          ...
          <button onClick={onBack} className="px-3 py-2 rounded-lg text-xs border border-white/[0.08] text-[#4B5563] hover:text-white">
            ← Retour
          </button>
        </div>
        <div className="mt-4 pt-4 border-t border-white/[0.06]">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {etudiant.nom_pere && <span className="text-[#4B5563]">Père : <span className="text-white">{etudiant.nom_pere}</span></span>}
            {etudiant.nom_mere && <span className="text-[#4B5563]">Mère : <span className="text-white">{etudiant.nom_mere}</span></span>}
      ...
      {etudiant.statut === 'suspendu' && (
        <div className="rounded-xl border border-[#F01F38]/30 p-4" style={{ background: 'rgba(248,81,73,0.06)' }}>
          ...
          <p className="text-xs text-[#4B5563] mt-1 ml-6">Veuillez vous rapprocher de l&apos;administration pour régulariser la situation.</p>
        </div>
      )}
        ...
        <>
              ...
              return (
                <button key={t.id} onClick={() => setActiveTab(t.id)}
                  ...
                  {t.count > 0 && activeTab !== t.id && (
                    <span className="ml-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-white/[0.08] text-[#4B5563]">{t.count}</span>
                  )}
                </button>
              )
            })}
          ...
          {activeTab === 'notes' && (
            <div className="space-y-4">
              {notes.length === 0 ? (
                <div className="text-center py-10 text-[#4B5563] text-xs">Aucune note disponible.</div>
              ) : PERIODES.map(p => {
                ...
                return (
                  <div key={p.value} className="rounded-xl border border-white/[0.06] overflow-hidden">
                    ...
                    <table className="w-full text-xs">
                      <thead><tr style={{ background: 'rgba(255,255,255,0.02)' }}>{['Matière', 'Note', '/Max', 'Moy /20', 'Coeff.'].map(h => <th key={h} className="text-left px-4 py-2 text-[10px] text-[#4B5563]">{h}</th>)}</tr></thead>
                      <tbody>
                        {pNotes.map(n => {
                ...
                {notifs.map(n => (
                  <div key={n.id} className={`rounded-xl border p-3 ${n.read ? 'border-white/[0.04] opacity-60' : 'border-[#F0A30A]/30'}`} style={{ background: n.read ? 'rgba(255,255,255,0.01)' : 'rgba(240,163,10,0.04)' }}>
                    <div className="flex items-start gap-2">
                      <Bell size={12} className={n.read ? 'text-[#4B5563]' : 'text-[#F0A30A]'} />
...
export default function EspaceParentPage() {
  ...
  if (tenantLoading || initLoading) {
    return <div className="flex items-center justify-center h-64 text-[#4B5563]"><Loader2 className="animate-spin mr-2" size={18} /> Chargement…</div>
  }
  ...
  if (isParentRole) {
        ...
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-[#4B5563] mt-0.5">Dossier de votre enfant</p>
            </div>
            {children.length > 1 && (
              <button onClick={() => setSelected(null)} className="flex items-center gap-1.5 text-xs text-[#4B5563] hover:text-white border border-white/[0.06] px-3 py-2 rounded-lg">
    ...
    return (
      <div className="space-y-5">
        <div>
          <h1 className="text-xl font-bold text-white">Espace Parent</h1>
          <p className="text-xs text-[#4B5563] mt-0.5">Suivi de vos enfants</p>
        </div>
        ...
        {children.length === 0 ? (
          ...
          <div className="space-y-3">
            <p className="text-xs font-bold text-[#4B5563] uppercase tracking-wider">{children.length} enfant{children.length > 1 ? 's' : ''}</p>
            {children.map(etu => (
              <button key={etu.id} onClick={() => setSelected(etu)}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-[#4B5563]">{etu.numero_id} · {etu.classe ?? NIVEAUX.find(n => n.value === etu.niveau)?.label} · {etu.annee_scolaire}</p>
                  <div className="mt-1"><StatutBadge statut={etu.statut} /></div>
                </div>
                <ChevronRight size={16} className="text-[#4B5563] group-hover:text-[#EC4899] transition-colors" />
              </button>
            ))}
          </div>
        )}
  ...
  if (selected) {
    return (
      <div className="space-y-5">
        <div>
          <h1 className="text-xl font-bold text-white">Espace Parent</h1>
          <p className="text-xs text-[#4B5563] mt-0.5">Administration · Dossier famille</p>
        </div>
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-white">Espace Parent</h1>
        <p className="text-xs text-[#4B5563] mt-0.5">Recherche par téléphone, email, nom du parent ou de l&apos;enfant</p>
      </div>
      ...
      <div className="rounded-xl border border-[#EC4899]/20 p-4 space-y-3" style={{ background: 'rgba(236,72,153,0.04)' }}>
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              className="w-full pl-9 pr-4 py-2.5 bg-white/[0.04] border border-white/[0.08] rounded-xl text-sm text-white placeholder-[#9CA3AF] focus:outline-none focus:border-[#EC4899]/50"
      ...
      {results.length > 0 && (
        <div className="rounded-xl border border-white/[0.06] overflow-hidden">
          <div className="px-4 py-3 border-b border-white/[0.06]" style={{ background: 'rgba(255,255,255,0.02)' }}>
            <p className="text-xs font-bold text-[#4B5563] uppercase tracking-wider">{results.length} résultat(s)</p>
          </div>
          {results.map(e => (
            <button key={e.id} onClick={() => setSelected(e)}
              ...
              <ChevronRight size={14} className="text-[#4B5563] group-hover:text-[#EC4899] transition-colors" />
            </button>
          ))}
        </div>
      )}
