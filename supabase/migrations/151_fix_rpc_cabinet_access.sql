-- ============================================================
-- Migration 151 — Fix accès RPC Finance pour utilisateurs cabinet
--
-- PROBLÈME :
--   Les 4 fonctions RPC finance vérifient :
--     SELECT 1 FROM profiles WHERE user_id = auth.uid() AND tenant_id = p_tenant_id
--   Ce check échoue pour les utilisateurs cabinet (ex: adjigordon) car leur
--   profiles.tenant_id pointe vers le tenant cabinet, PAS vers le tenant client
--   (AMD FINANCE, CNSS Congo, etc.).
--
-- FIX :
--   1. Créer fn_check_tenant_access() — helper réutilisable
--      Accepte : accès direct (profil sur le tenant) OU accès cabinet
--      (cabinet_clients.client_tenant_id = p_tenant_id)
--   2. Remplacer le check dans les 4 fonctions par PERFORM fn_check_tenant_access()
--
-- COLONNES RÉELLES (rappel, confirmé migration 150) :
--   transactions.date          (pas date_operation)
--   factures.created_at::DATE  (pas date — migration 010 non appliquée en prod)
-- ============================================================

-- ── 0. Helper d'accès tenant ─────────────────────────────────────────────────
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND tenant_id = p_tenant_id) THEN
    RETURN; -- accès direct OK
  END IF;

  IF EXISTS (
    SELECT 1
    FROM cabinet_clients cc
    INNER JOIN profiles p ON p.tenant_id = cc.cabinet_tenant_id
    WHERE p.user_id        = auth.uid()
      AND cc.client_tenant_id = p_tenant_id
      AND cc.statut           = 'actif'
  ) THEN
    RETURN; -- accès cabinet OK
  END IF;

  RAISE EXCEPTION 'Access denied: aucun accès au tenant %', p_tenant_id;
END;
$$;

-- ── 1. DROP fonctions existantes ─────────────────────────────────────────────
...
BEGIN
  ...
  -- CA depuis factures payées (created_at car colonne date absente — mig 010 non appliquée)
  ...
  -- Entrées transactions hors paie (transactions.date = colonne réelle)
  SELECT
...
BEGIN
  ...
  -- CA 90 derniers jours (factures.created_at car date absente)
...
-- ── FIN Migration 151 ────────────────────────────────────────────────────────
-- Accès autorisé si :
--   A) profiles.user_id = auth.uid() ET profiles.tenant_id = p_tenant_id  (direct)
--   B) cabinet_clients.client_tenant_id = p_tenant_id ET
--      profiles.tenant_id = cabinet_clients.cabinet_tenant_id ET
--      profiles.user_id = auth.uid() ET cabinet_clients.statut = 'actif'  (cabinet)
