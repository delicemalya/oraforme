...
CREATE TABLE IF NOT EXISTS banque_membres (
  ...
  type_compte    TEXT NOT NULL DEFAULT 'epargne'
                   CHECK (type_compte IN ('epargne','courant','depot_terme','credit')),
...
-- Crédits : montant_demande + montant_accorde séparés pour traçabilité
CREATE TABLE IF NOT EXISTS banque_credits (
  ...
  statut            TEXT NOT NULL DEFAULT 'en_attente'
                      CHECK (statut IN ('en_attente','accorde','en_cours','solde','refuse')),
...
-- Opérations : type_operation (pas "type" qui est un mot réservé SQL)
CREATE TABLE IF NOT EXISTS banque_operations (
  id             UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id      UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  membre_id      UUID REFERENCES banque_membres(id) ON DELETE SET NULL,
  type_operation TEXT NOT NULL
                   CHECK (type_operation IN ('depot','retrait','virement','remboursement','interet','frais')),
...
CREATE INDEX IF NOT EXISTS idx_banque_ops_tenant ON banque_operations(tenant_id, date_operation DESC);
CREATE INDEX IF NOT EXISTS idx_banque_ops_membre ON banque_operations(membre_id, date_operation DESC);
...
-- Intrants : type_intrant + quantite_stock + date_achat/expiration/notes
CREATE TABLE IF NOT EXISTS agriculture_intrants (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id       UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  nom             TEXT NOT NULL,
  type_intrant    TEXT NOT NULL
                    CHECK (type_intrant IN ('semence','engrais','pesticide','herbicide','equipement','carburant','autre')),
  quantite_stock  NUMERIC(12,2) DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_agri_intrants_tenant ON agriculture_intrants(tenant_id, type_intrant);
