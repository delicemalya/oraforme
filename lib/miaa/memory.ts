/**
 * lib/miaa/memory.ts — Système de mémoire MIAA PREMIUM
 * Charge le contexte complet de l'entreprise pour alimenter le prompt.
 */
...
export async function mettreAJourMemoire(
  ...
  try {
    const { data: existing } = await supabase
      .from('miaa_memory').select('historique_resume, nb_conversations, patterns')
      .eq('tenant_id', tenantId).maybeSingle()

    const resume = existing?.historique_resume ?? ''
    const fragment = `\nQ: ${question.slice(0, 120)}\nR: ${reponse.slice(0, 250)}`
    const nouveauResume = resume.length > 2500
      ? resume.slice(-2000) + fragment
      : resume + fragment

    await supabase.from('miaa_memory').upsert({
      tenant_id:        tenantId,
      historique_resume: nouveauResume,
      nb_conversations:  (existing?.nb_conversations ?? 0) + 1,
