...
export default function KpiCard({
  ...
  return (
    <motion.div
      ...
      className="relative bg-[var(--card-bg)] border border-[var(--border)] rounded-xl p-4 overflow-hidden cursor-default select-none"
    ...
    >
      ...
      <p className="text-[10px] text-[var(--text-secondary)] mt-1.5 uppercase tracking-wider font-semibold relative">{label}</p>
      {sub && <p className="text-[10px] text-[var(--text-secondary)] mt-0.5 relative truncate">{sub}</p>}
    </motion.div>
  )
}
