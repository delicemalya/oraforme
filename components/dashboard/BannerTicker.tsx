...
export function BannerTicker() {
  ...
  // Build slides once weather is loaded
  useEffect(() => {
    ;(async () => {
      ...
      const built: Slide[] = []
      built.push({ icon: weather.emoji, text: `${weather.temp}°C · ${weather.ville}` })
      built.push({
        icon: '📅',
  ...
  // Cycle every 5 s : fade out → change → fade in
  useEffect(() => {
        ...
        {THEMES.slice(0, 5).map(t => (
          <button key={t.id} onClick={() => changerTheme(t.id)} title={t.nom}
            ...
            style={{
              ...
              border: theme.id === t.id ? '2px solid #fff' : '2px solid rgba(255,255,255,0.4)',
              transform: theme.id === t.id ? 'scale(1.25)' : undefined,
            }}
          />
        ))}

        {/* Bouton + pour les 5 autres thèmes */}
        <button onClick={() => setPicker(v => !v)} title="Tous les thèmes"
          className="w-[18px] h-[18px] rounded-full flex items-center justify-center text-white text-[10px] font-bold transition-all hover:scale-110"
              ...
              <div className="grid grid-cols-5 gap-2 mb-3">
                {THEMES.map(t => (
                  <button key={t.id} onClick={() => { changerTheme(t.id); setPicker(false) }}
                    ...
                    {theme.id === t.id && <span className="text-white text-[11px] font-bold">✓</span>}
                  </button>
                ))}
              </div>
