...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-xl p-3 shadow-2xl text-xs">
      <p className="text-[#111827] font-semibold mb-1 truncate max-w-[160px]">{label}</p>
      <p className="text-[#F0A30A]">{fmtFull(payload[0].value)}</p>
      {payload[0].payload.count && (
        <p className="text-[#6B7280] mt-0.5">{payload[0].payload.count} facture(s)</p>
      )}
    </div>
  )
}
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h3 className="text-sm font-bold text-[#111827]">Top 5 Clients</h3>
          <p className="text-[10px] text-[#6B7280] mt-0.5">Par chiffre d'affaires ce mois</p>
        </div>
      </div>
            ...
            {data.slice(0, 5).map((c, i) => (
              <div key={i} className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[#4B5563] truncate max-w-[120px]">{c.nom}</span>
                </div>
                <span className="text-[#111827] font-semibold">{fmtFull(c.montant)}</span>
              </div>
            ))}
          </div>
        </>
